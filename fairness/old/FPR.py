...
def FPR(dataset_dir, percentage=100, use_multiprocessing=False, num_cores=None):
    ...
    image_counts = {
        folder: len([img for img in os.listdir(os.path.join(dataset_dir, folder)) if img.endswith('.jpg')])
    ...
    image_pairs = [
        ...
        for folder1, folder2 in itertools.combinations(subfolders, 2)
        for img1 in os.listdir(os.path.join(dataset_dir, folder1)) if img1.endswith('.jpg')
        for img2 in os.listdir(os.path.join(dataset_dir, folder2)) if img2.endswith('.jpg')
    ]
